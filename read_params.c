 ...
 *
 * $Id: read_params.c 4627 2008-10-01 16:48:11Z markstro $
 *
   $Revision: 4627 $
        $Log: read_params.c,v $
...
*/
	...
	if (index == 0) {  // if index equals zero, than this parameter file has dimension stuff and we need to skip over it.
		while (strncmp (line, "** Parameters **", 16)) {
			if (!fgets (line, MAXLNLEN, param_file)) {  // return if hits eol
				fclose (param_file);
				return (NULL);
			}
		}
...
*/
   ...
   if (isdigit(*line)) {
      if ((*param_ptr)->ndimen != atol(line)) {
         (void)sprintf (buf,
            "read_params: number of dimensions for %s in parameter file doesn't match parameter declaration in module.\n", key);
         return (buf);
      }
...
*/
      ...
      for (i = 0; i < (*param_ptr)->ndimen; i++) {
        ...
        if (strcmp(dimen, (*param_ptr)->dimen[i]->name)) {
           (void)sprintf (buf,
                 "read_params: expecting dimension: %s\nread dimension: %s\nkey is: %s\nfile: %s",
                 dimen, (*param_ptr)->dimen[i]->name, key, param_file_name);
           return (buf);
        }
     } /* i */
...
*/
	...
	(void)sprintf (buf, "read_params: size incorrect, key:%s\nfile: %s",
		       key, param_file_name);
	return (buf);
      }
    } else {
...
*/
    ...
    if((type = atol(line)) == 0) {
      (void)sprintf (buf, "read_params: decoding type from:%s\nkey: %s\nfile: %s",
		     line, key, param_file_name);
      return (buf);
    }
    if(type != (*param_ptr)->type) {
      (void)sprintf (buf, "read_params: type incorrect, key:%s\nfile: %s",
		     key, param_file_name);
      return (buf);
    }
...
static char *READ_param_values (PARAM *param, FILE *param_file, char line[]) {
	...
	char *result = NULL;
	char *comp_ptr = NULL;
	static char crap[MAXLNLEN], crap2[MAXLNLEN];
	static char buf[256];
	float foo;
...
*/
	...
	if (i < param->size) {
		sprintf (buf, "READ_param_values: too FEW values read. param = %s read_count = %d size = %d\n", param->name, i, param->size);
		printf ("%s", buf);
		return (buf);
		//printf ("READ_param_values: too few values read. param = %s read_count = %d size = %d\n", param->name, i, param->size);
	} else if (i > param->size) {
		sprintf (buf, "READ_param_values: too MANY values read. param = %s read_count = %d size = %d\n", param->name, i, param->size);
		printf ("%s", buf);
		return (buf);
	}
