...
set BASIN=andcrk
REM *****set "WEB_DAT=%HOME_BATCH%\..\WEBDATA"
REM *****set "WEB_PEST=%HOME_BATCH%\..\WEBTPL"
REM *****set "WEB_SRC=.\webfiles"
REM *****set "PROJECT_DIR_PATH=%HOME_BATCH%\webmod"
REM *****set "PROJECT_DIR=%HOME%/webmod"
REM *****set "PEST_BIN_DIR_PATH=%HOME_BATCH%\..\..\PC_bin"
REM *****set "PEST_BIN_DIR=%HOME%/../../PC_bin"
REM *****set "TSPROC_BIN_DIR=%HOME%/../../PC_bin"
set "WEB_DAT_INPUT=%HOME_BATCH%\..\WEBDATA"
set "WEB_DAT_OUTPUT=%HOME_BATCH%\..\WEBDATA"
set "WEB_DAT_CONTROL=%HOME_BATCH%\..\WEBDATA"
set "WEB_PEST=%HOME_BATCH%\..\WEBPEST"
...
mkdir %PROJECT_DIR_PATH%\input
mkdir %PROJECT_DIR_PATH%\output
mkdir %PROJECT_DIR_PATH%\control

REM Sed to make basin_tsproc.dat 
sed "s#@PROJECT_DIR@#%PROJECT_DIR%/#" %WEB_PEST%\%BASIN%_tsproc.tpl > %PROJECT_DIR%\%BASIN%_tsproc.dat
sed -i "s#@PROJECT_DIR@#%PROJECT_DIR%/#" %PROJECT_DIR%\%BASIN%_tsproc.dat

REM Sed to make pest_webmod.bat in PROJECT_DIR
sed "s#@PROJECT_DIR@#%PROJECT_DIR%/#g" %WEB_PEST%\pest_webmod.bat.tpl > %PROJECT_DIR_PATH%\pest_webmod.bat
...
copy %WEB_PEST%\*.ssf            	%PROJECT_DIR_PATH%
copy %WEB_PEST%\pest_groups*.txt 	%PROJECT_DIR_PATH%
copy %WEB_PEST%\pest_params*.txt 	%PROJECT_DIR_PATH%
copy %WEB_PEST%\tsproc.in        	%PROJECT_DIR_PATH%
copy %WEB_PEST%\%BASIN%.statvar.full	%PROJECT_DIR_PATH%\%BASIN%.statvar
copy %WEB_PEST%\par2par_%BASIN%.tpl	%PROJECT_DIR_PATH%
...
cp %WEB_DAT_CONTROL%\%BASIN%.control    %PROJECT_DIR_PATH%/control
...
cd %PROJECT_DIR_PATH%
start "Master" cmd /k "call %PEST_BIN_DIR_PATH%\beopest64 %PROJECT_DIR_PATH%\%pst% /H /L :%PORT% & cd .."
...
for /l %%X in (1, 1, %nodes%) do (
	...
	mkdir input
	mkdir control
	copy %WEB_PEST%\params_%BASIN%.tpl .\
	copy %WEB_PEST%\pqi_%BASIN%.tpl .\
	copy %WEB_DAT_INPUT%\%BASIN%.dat .\input
	copy %WEB_DAT_INPUT%\%BASIN%.dat.chemdat .\input
	copy %WEB_DAT_INPUT%\phreeq_lut .\input
	copy %WEB_DAT_INPUT%\phreeqc_web_lite.dat .\input
