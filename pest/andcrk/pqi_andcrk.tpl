...
Pyrite_O2_xtreme
  -start
   1 rem        Handcart Gulch model numbers, round 1, Linear relation of water table and pyrite weathering rate
   2 rem        parm(1) = log10(A/V, m^2/L water)      parm(2) = exp for (m/m0)
   3 rem        parm(3) = exp for O2            parm(4) = exp for H+
   4 rem        parm(5) threshold of gwsto above which pyrite weathering is enhanced
#   10 if (m <= 0) then goto 200
   20 if (si("Pyrite") >= 0) then goto 200
...
#
# For run _315 Change xtreme to be enhanced when level above baseline not below.
#
# linear enhancement when gwsto above parm(5)
   28 if(gwsto > parm(5)) then rate = rate * (gwsto-parm(5)) else rate = 0
   30  moles = rate * time
#   40 if (moles > m) then moles = m
   200 save moles
  -end
