...
int EndRow(void)
{
	if (CSelectedOutput::singleton.GetRowCount() <= 1) {
		// ensure all user_punch headings are included
		for (int i = phreeqc::n_user_punch_index; i < phreeqc::user_punch_count_headings; ++i) {
			CSelectedOutput::singleton.PushBackEmpty(phreeqc::user_punch_headings[i]);
		}
	}
	return CSelectedOutput::singleton.EndRow();
}

int PushBackDouble(const char* key, double dVal)
{
	return CSelectedOutput::singleton.PushBackDouble(key, dVal);
}

int PushBackLong(const char* key, long lVal)
{
	return CSelectedOutput::singleton.PushBackLong(key, lVal);
}

int PushBackString(const char* key, const char* sVal)
{
	return CSelectedOutput::singleton.PushBackString(key, sVal);
}

int PushBackEmpty(const char* key)
{
	return CSelectedOutput::singleton.PushBackEmpty(key);
}
...
void AddSelectedOutput(const char* name, const char* format, va_list argptr)
{
	...
	*/
	...
	if (bDouble) {
		...
		CSelectedOutput::singleton.PushBackDouble(name, valDouble);
	}
	else if (bInt) {
		int valInt;
		valInt = va_arg(argptr, int);

		CSelectedOutput::singleton.PushBackLong(name, (long)valInt);
	}
	else if (bString) {
		char* valString;
		valString = (char *)va_arg(argptr, char *);

		CSelectedOutput::singleton.PushBackString(name, valString);
	}
	else {
		ASSERT(false);
		CSelectedOutput::singleton.PushBackEmpty(name);
	}
}

CSelectedOutput CSelectedOutput::singleton;
