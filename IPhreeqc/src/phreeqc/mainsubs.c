...
static char const rcsid[] = "$RCSfile: mainsubs.c,v $  $Revision: 2.30 $";
...
extern int add_isotopes(struct solution *solution_ptr);
extern int check_residuals(void);
extern int clean_up(void);
extern void cmd_initialize(void);
extern int copier_init(struct copier *copier_ptr);
...
extern struct exchange *exchange_bsearch (int k, int *n);
extern int exchange_compare (const void *ptr1, const void *ptr2);
extern int exchange_duplicate(int n_user_old, int n_user_new);
...
extern struct gas_phase *gas_phase_bsearch (int n_user, int *n);
extern int gas_phase_compare (const void *ptr1, const void *ptr2);
extern int gas_phase_duplicate(int n_user_old, int n_user_new);
extern int gas_phase_free (struct gas_phase *gas_phase_ptr);
extern int get_elts_in_species (char **t_ptr, LDBLE coef);
extern struct irrev *irrev_bsearch (int k, int *n);
extern int irrev_duplicate(int n_user_old, int n_user_new);
extern struct kinetics *kinetics_bsearch (int k, int *n);
extern int kinetics_duplicate(int n_user_old, int n_user_new);
extern int hcreate_multi(unsigned Count, HashTable **HashTable_ptr);
...
extern int mix_duplicate(int n_user_old, int n_user_new);
extern int model (void);
extern struct pe_data *pe_data_dup (struct pe_data *pe_ptr_old);
extern int pe_data_free (struct pe_data *pe_data_ptr);
extern struct pp_assemblage *pp_assemblage_bsearch (int k, int *n);
extern int pp_assemblage_compare(const void *ptr1, const void *ptr2);
extern int pp_assemblage_duplicate(int n_user_old, int n_user_new);
...
extern int print_exchange (void);
extern int print_gas_phase(void);
extern int print_surface(void);
extern int punch_all(void);
extern int read_input(void);
extern int read_log_k_only (char *ptr, LDBLE *log_k);
...
extern struct s_s_assemblage *s_s_assemblage_bsearch (int n_user, int *n);
extern int s_s_assemblage_compare (const void *ptr1, const void *ptr2);
extern int s_s_assemblage_duplicate(int n_user_old, int n_user_new);
extern int s_s_assemblage_free (struct s_s_assemblage *s_s_assemblage_ptr);
extern int set(int initial);
extern int set_and_run_wrapper(int i, int use_mix, int use_kinetics, int nsaver, LDBLE step_fraction);
extern int set_initial_moles(int i);
...
extern int species_list_sort(void);
...
extern int sum_diffuse_layer(struct surface_charge *surface_charge_ptr);
extern int sum_species(void);
extern struct surface *surface_bsearch (int k, int *n);
extern struct surface *surface_bsearch (int k, int *n);
extern int surface_compare (const void *ptr1, const void *ptr2);
extern int surface_delete(int n_user_old);
extern int surface_duplicate(int n_user_old, int n_user_new);
extern int surface_free(struct surface *surface_ptr);
extern int surface_model(void);
extern struct temperature *temperature_bsearch (int k, int *n);
extern int temperature_duplicate(int n_user_old, int n_user_new);
...
/* ---------------------------------------------------------------------- */
{
	...
	new_file = fopen(default_name, status);
	if (new_file == NULL) {
#if defined(PHREEQCI_GUI)
		sprintf(error_string, "Can't open file, %s.\r\n", default_name);
		SendMessage(g_status.hText, EM_REPLACESEL, (WPARAM)0, (LPARAM)error_string);
		sprintf(g_szLineBuf, "Stopping.\r\n");
		SendMessage(g_status.hText, EM_REPLACESEL, (WPARAM)0, (LPARAM)g_szLineBuf);
		RaiseException(ERROR_SUCCESS, 0, 0, NULL);
#endif
	}
#else 
	for (;;) {
