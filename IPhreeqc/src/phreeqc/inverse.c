...
static char const rcsid[] = "$RCSfile: inverse.c,v $  $Revision: 2.6 $";
...
extern int add_solution (struct solution *solution_ptr, LDBLE extensive, 
		  LDBLE intensive);
extern int array_print(LDBLE *array_l, int row_count, int column_count, int max_column_count1);
extern LDBLE calc_alk(struct reaction *rxn_ptr);
extern int cl1(int k, int l, int m, int n,
	int nklmd_l, int n2d_l,
	LDBLE *q,
	int *kode_l, LDBLE toler_l,
	int *iter_l, LDBLE *x_l, LDBLE *res_l, LDBLE *error_l,
	LDBLE *cu_l, int *iu_l, int *s_l);
