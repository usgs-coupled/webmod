...
static char const rcsid[] = "$RCSfile: isotopes.c,v $  $Revision: 2.9 $";
...
extern char *sformatf(const char* format, ...);
 ...
 */
	...
	for (j = 0; j < count_isotope_alpha; j++) {
		...
		if (isotope_alpha[j]->named_logk != NULL) {
			output_msg(OUTPUT_MESSAGE,"%-37s%14.5g%14.5g%14.5g\n", token, (double) isotope_alpha[j]->value, (double) (1000*log(isotope_alpha[j]->value)), (double) (1000*calc_logk_n(isotope_alpha[j]->named_logk)*LOG_10));
		} else {
