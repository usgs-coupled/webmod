...
static char const rcsid[] = "$RCSfile: print.c,v $  $Revision: 2.31 $";
...
extern LDBLE calc_surface_charge(char *surface_name);
...
extern int equal (LDBLE a, LDBLE b, LDBLE eps);
...
extern int print_initial_solution_isotopes(void);
extern int print_isotope_ratios(void);
extern int print_isotope_alphas(void);
extern int print_calculate_values(void);
extern int punch_isotopes(void);
extern int punch_calculate_values(void);
extern int replace(const char *str1, const char *str2, char *str);
extern int rewrite_master_to_secondary(struct master *m1, struct master *m0);
...
#endif
	if (punch_file != NULL && pr.punch == TRUE && punch.in == TRUE)	{
		va_start(args, format);
		retval = vfprintf(punch_file, format, args);
		va_end(args);
#if defined(USE_MPI) && defined(HDF5_CREATE) && defined(MERGE_FILES)
		va_start(args, format);
...
int fpunchf_user(int user_index, const char* format, ...)
{
	...
	if (punch_file != NULL && punch.in == TRUE && pr.punch == TRUE) {
		va_start(args, format);
		retval = vfprintf(punch_file, format, args);
		va_end(args);
#if defined(USE_MPI) && defined(HDF5_CREATE) && defined(MERGE_FILES)
