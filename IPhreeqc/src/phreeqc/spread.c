...
static char const rcsid[] = "$RCSfile: spread.c,v $  $Revision: 2.14 $";
...
extern int check_units (char *tot_units,  int alkalinity, int check_compatibility, const char *default_units, int  print);
...
extern int find_option(char *item, int *n, const char **list, int count_list, int exact);
...
extern int read_conc(int n, int count_mass_balance, char *next_char);
extern int read_number_description (char *ptr, int *n_user, int *n_user_end, char **description);
