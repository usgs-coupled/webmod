...
static char const rcsid[] = "$RCSfile: readtr.c,v $  $Revision: 2.8 $";
...
extern int get_true_false(char *string, int default_value);
...
extern int *read_list_ints_range (char **ptr, int *count_ints, int positive, int *int_list );
extern int read_number_description (char *ptr, int *n_user, int *n_user_end,
			     char **description);
 ...
 */
			...
			dump_in = TRUE;
#ifdef PHREEQCI_GUI
			if (copy_lfn_gui(file_name, &next_char, &l) == EMPTY) {
				strcpy(file_name, "phreeqc.dmp");
			}
#else
 ...
 */
	...
	if (dump_in == TRUE) {
		...
		if (error_checking_gui() == TRUE) {
			/* TODO check for access and valid name */
