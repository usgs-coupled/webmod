...
static char const rcsid[] = "$RCSfile: tally.c,v $  $Revision: 1.11 $";
/*
 *   External routines
 */
extern int add_elt_list(struct elt_list *elt_list_ptr, LDBLE coef);
extern int add_exchange (struct exchange *exchange_ptr);
extern int add_gas_phase (struct gas_phase *gas_phase_ptr);
extern int add_kinetics (struct kinetics *kinetics_ptr);
extern int add_pp_assemblage (struct pp_assemblage *pp_assemblage_ptr);
extern int add_reaction (struct irrev *irrev_ptr, int step_number, LDBLE step_fraction);
extern int add_solution (struct solution *solution_ptr, LDBLE extensive, LDBLE intensive);
extern int add_surface (struct surface *surf_ptr);
extern int add_s_s_assemblage (struct s_s_assemblage *s_s_assemblage_ptr);
...
extern int string_trim_right (char *str);
...
extern int tidy_punch(void);
extern int warning_msg (const char *err_str);
extern int xsolution_zero (void);
/*
 *  storage
 */
struct buffer {
...
int get_tally_table_column_heading(int column, int *type, char *string, size_t string_length);
int get_tally_table_row_heading(int column, char *string, size_t string_length);
...
/* ---------------------------------------------------------------------- */
int get_tally_table_row_heading(int row, char *string, size_t string_length) 
/* ---------------------------------------------------------------------- */
{
	/*
	 *  row is C row number
	...
	strncpy(string, buffer[row].name, string_length);
	return(OK);
}
/* ---------------------------------------------------------------------- */
int get_tally_table_column_heading(int column, int *type, char *string, size_t string_length) 
/* ---------------------------------------------------------------------- */
{
	...
	*type = -1;
	...
	strncpy(string, tally_table[column].name, string_length);
