...
static char const rcsid[] = "$RCSfile: tidy.c,v $  $Revision: 2.26 $";
...
extern struct calculate_value  *calculate_value_search (char *name);
extern LDBLE calc_alk(struct reaction *rxn_ptr);
extern int check_eqn ( int association );
extern LDBLE coef_in_master(struct master *master_ptr);
extern int conc_compare(const void *ptr1, const void *ptr2);
extern int copy_token (char *token_ptr, char **ptr, int *l);
extern int compute_gfw(const char *string, LDBLE *gfw);
...
extern int gas_phase_sort(void);
...
extern struct isotope_ratio  *isotope_ratio_search (char *name);
extern LDBLE k_calc(LDBLE *logk, LDBLE tempk);
extern struct kinetics *kinetics_bsearch (int k, int *n);
extern int kinetics_duplicate(int n_user_old, int n_user_new);
extern int kinetics_sort(void);
extern struct logk  *logk_search (char *name);
...
extern int master_compare (const void *ptr1, const void *ptr2);
struct master_isotope  *master_isotope_search (char *name);
#ifdef SKIP
extern int mix_compare (const void *ptr1, const void *ptr2);
#endif
extern int match_elts_in_species (char *name, char *template);
extern int mix_sort(void);
extern struct phase *phase_bsearch (char *name, int *i, int print);
extern int phase_compare(const void *ptr1, const void *ptr2);
...
extern int rxn_free(struct reaction *rxn_ptr);
extern int s_compare (const void *ptr1, const void *ptr2);
...
extern int s_s_assemblage_sort(void);
void str_tolower(char *str);
extern int strcmp_nocase(const char *str1, const char *str2);
extern char *string_hsave (const char *str);
extern int surface_comp_compare(const void *ptr1, const void *ptr2);
extern int surface_compare (const void *ptr1, const void *ptr2);
extern int surface_sort(void);
extern int temperature_duplicate(int n_user_old, int n_user_new);
extern int temperature_sort(void);
extern int trxn_add(struct reaction *r_ptr, LDBLE coef, int combine);
extern int trxn_combine (void);
extern int trxn_copy (struct reaction *rxn_ptr);
extern int trxn_reverse_k(void);
