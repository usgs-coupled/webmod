...
static char const rcsid[] = "$RCSfile: kinetics.c,v $  $Revision: 2.26 $";
#ifdef SKIP
...
extern void odeint(LDBLE ystart[], int nvar, LDBLE x1, LDBLE x2, LDBLE eps, LDBLE h1,
 	LDBLE hmin, int *nok, int *nbad, int n_user, int nsaver);
#endif
...
static int set_advection(int i, int use_mix, int use_kinetics, int nsaver);
...
extern int backspace_screen (int spaces);
extern int basic_compile(char *commands, void **lnbase, void **vbase, void **lpbase);
extern int basic_run(char *commands, void *lnbase, void *vbase, void *lpbase);
extern int basic_main(char *commands);
extern int check_residuals(void);
extern int dump_exchange(int k);
extern int dump_gas_phase(int k);
extern int dump_kinetics (int k);
extern int dump_mix (int k);
extern int dump_pp_assemblage(int k);
extern int dump_reaction (int k);
extern int dump_solution (int k);
extern int dump_surface(int k);
extern int dump_s_s_assemblage (int k);
...
extern struct gas_phase *gas_phase_bsearch (int k, int *n);
extern int gas_phase_copy(struct gas_phase *gas_phase_old_ptr, 
...
extern int kinetics_copy(struct kinetics *kinetics_old_ptr, 
...
extern struct phase *phase_bsearch (char *ptr, int *j, int print);
extern struct pp_assemblage *pp_assemblage_bsearch (int k, int *n);
extern int pp_assemblage_copy(struct pp_assemblage *pp_assemblage_old_ptr, 
...
extern struct s_s_assemblage *s_s_assemblage_bsearch (int k, int *n);
extern int s_s_assemblage_copy(struct s_s_assemblage *s_s_assemblage_old_ptr, 
...
extern int step(LDBLE step_fraction);
...
extern int system_duplicate (int i, int save_old);
extern struct temperature *temperature_bsearch (int k, int *n);
extern int xsolution_save(int k_user);
 ...
 */
	...
	if (use_mix == DISP) {
		...
		use.mix_in = TRUE;
		use.n_mix_user = i;
	} else if (use_mix == STAG || use_mix == TRUE) {
 ...
 */ 
		...
		if( (dump_file = fopen("error.inp", "w")) == NULL) {
