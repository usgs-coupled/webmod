...
static char const rcsid[] = "$RCSfile: structures.c,v $  $Revision: 2.28 $";
...
static int exchange_copy(struct exchange *exchange_old_ptr, 
...
#ifdef SKIP
static struct phase *phase_search (char *ptr, int *n);
#endif
...
int pp_assemblage_init (int n, int n_user);
...
extern int clean_up_message(void);
...
extern void free_hash_strings(HashTable *Table);
...
extern int calculate_value_free(struct calculate_value *calculate_value_ptr);
...
/* free user database name if defined */
	...
	selected_output_file = free_check_null(selected_output_file);
#ifdef PHREEQCI_GUI
...
/* message callbacks */
	...
	PHRQ_free_all();
/* files */
/* database_file closed in main */
	if (input != NULL) fclose (input);
	if (output != NULL) fclose (output);
	if (log_file != NULL) fclose (log_file);
	if (punch_file != NULL) fclose (punch_file);
...
#if defined(PHREEQCI_GUI)
	input = output = log_file = punch_file = dump_file = error_file = NULL;
#endif
	return(OK);
}
/* **********************************************************************
 ...
 *      return: OK
 */
	pp_assemblage[n].n_user = n_user;
	pp_assemblage[n].pure_phases = PHRQ_malloc((size_t) sizeof(struct pure_phase));
	if (pp_assemblage[n].pure_phases == NULL) malloc_error();

	pp_assemblage[n].next_elt = NULL;
