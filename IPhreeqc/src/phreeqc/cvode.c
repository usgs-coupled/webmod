...
#include "message.h"
#include "kinetics.h"
static char const rcsid[] = "$RCSfile: cvode.c,v $  $Revision: 1.6 $";
          ...
          realtype *t, int itask)
{
  ...
  *t = tn;
  ...
  loop {
    ...
    /*
      output_msg(OUTPUT_CVODE, "tn %e, t0 %e, tout %e, step_frac %e\n", tn, t0, tout, cvode_step_fraction);
    */
...
#ifdef DEBUG_CVODE
  if (cvode_error == TRUE) {
	  foutput_msg(OUTPUT_CVODE,"CVYddNorm error\n");
  }
#endif
  nfe++;
...
#ifdef DEBUG_CVODE
	  foutput_msg(OUTPUT_CVODE,"Before predict, y OK, time %e\n", tn);
#endif
    }
#ifdef DEBUG_CVODE
    cvode_test = TRUE;
