...
static char const rcsid[] = "$RCSfile: model.c,v $  $Revision: 2.22 $";
...
extern int array_print(LDBLE *array_l, int row_count, int column_count, int max_column_count);
extern int calc_all_g(void);
extern int calc_init_g(void); 
extern int calculate_values(void);
extern int check_same_model(void);
extern int cl1(int k, int l, int m, int n,
	int nklmd, int n2d,
	LDBLE *q,
	int *kode, LDBLE toler,
	int *iter, LDBLE *x_l, LDBLE *res, LDBLE *error,
	LDBLE *cu, int *iu, int *s_l);
...
/* extern int ineq(int kode); */
extern int initial_surface_water(void);
...
/* debug
extern int print_species(void);
extern int print_surface(void);
*/
extern int reprep (void);
extern int status (int count, const char *str);
extern int switch_bases(void);
