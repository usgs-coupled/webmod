...
/*     $Date: 2004/02/17 15:24:34 $ */
static char const rcsid[] = "$RCSfile: read.c,v $  $Revision: 2.38 $";
...
extern int read_isotope_alphas (void);
extern int read_isotope_ratios (void);
...
extern int read_solution_spread(void);
...
extern int conc_compare(const void *ptr1, const void *ptr2);
extern int copier_add(struct copier *copier_ptr, int n_user, int start, int end);
extern int copy_token (char *token_ptr, char **ptr, int *l);
extern int dup_print(const char *ptr, int emphasis);
extern struct element *element_store (char *element);
extern struct elt_list *elt_list_save(void);
extern struct elt_list *elt_list_dup(struct elt_list *elt_list_ptr_old);
extern int equal (LDBLE a, LDBLE b, LDBLE n);
extern int error_msg (const char *err_str, const int stop);
extern int exchange_comp_compare(const void *ptr1, const void *ptr2);
extern int exchange_free (struct exchange *exchange_ptr);
extern struct exchange *exchange_search(int n_user, int *n, int print);
extern void *free_check_null(void *ptr);
extern int gas_comp_compare(const void *ptr1, const void *ptr2);
extern int gas_phase_free (struct gas_phase *gas_phase_ptr);
extern struct gas_phase *gas_phase_search (int n_user, int *n);
extern int get_elt (char **t_ptr, char *element, int *i);
extern int get_elts_in_species (char **t_ptr, LDBLE coef);
extern int get_num (char **t_ptr, LDBLE *num);
extern int get_secondary_in_species (char **t_ptr, LDBLE coef);
extern int get_token(char **eqnaddr, char *string, LDBLE *z, int *l);
extern ENTRY * hsearch_multi(HashTable *Table, ENTRY item, ACTION action);
extern struct inverse *inverse_alloc(void);
extern int inverse_delete (int i);
extern int inverse_isotope_compare (const void *ptr1, const void *ptr2);
extern struct inverse *inverse_search (int n_user, int *n);
extern int irrev_free (struct irrev *irrev_ptr);
extern struct irrev *irrev_search(int n_user, int *n);
extern int isotope_compare (const void *ptr1, const void *ptr2);
extern int kinetics_free (struct kinetics *kinetics_ptr);
extern struct kinetics *kinetics_search(int n_user, int *n, int print);
extern struct logk  *logk_store (char *name, int replace_if_found);
extern void malloc_error (void);
extern struct master *master_alloc(void);
extern int master_delete(char *ptr);
extern struct master *master_search (char *ptr, int *n);
extern int mix_free (struct mix *mix_ptr);
extern struct mix *mix_search(int n_user, int *n, int print);
extern int parse_eq (char *eqn, struct elt_list **elt_ptr, int association);
extern int parse_couple(char *token);
extern int pe_data_store (struct pe_data **pe, const char *token);
extern struct phase *phase_store (char *name);
extern int pp_assemblage_init (int n, int n_user);
extern struct pp_assemblage *pp_assemblage_search (int n_user, int *n);
extern int pp_assemblage_free (struct pp_assemblage *pp_assemblage_ptr);
extern int pure_phase_compare (const void *ptr1, const void *ptr2);
extern int rate_free(struct rate *rate_ptr);
extern struct rate *rate_search (char *name, int *n);
extern struct reaction *rxn_dup(struct reaction *rxn_ptr_old);
extern int read_isotopes(void);
extern int read_calculate_values (void);
extern int read_transport (void);
extern int replace(const char *str1, const char *str2, char *str);
extern struct reaction *rxn_alloc(int ntokens);
extern int s_s_compare(const void *ptr1, const void *ptr2);
extern int s_s_assemblage_free (struct s_s_assemblage *s_s_assemblage_ptr);
extern struct s_s_assemblage *s_s_assemblage_search (int n_user, int *n);
extern struct species *s_search ( char *name);
extern struct species  *s_store (char *name, LDBLE z, int replace_if_found);
extern struct solution *solution_alloc(void);
extern struct solution *solution_bsearch(int k, int *n, int print);
extern int solution_delete(int i);
extern int solution_free (struct solution *solution_ptr);
extern int solution_sort(void);
extern void space (void **ptr, int i, int *max, int struct_size);
extern void squeeze_white(char *s_l);
extern void str_tolower(char *str);
extern int strcmp_nocase( const char *nptr1, const char *nptr2 );
extern int strcmp_nocase_arg1(const char *str1, const char *str2);
extern char * string_duplicate (const char *token);
extern int string_trim (char *str);
extern char *string_hsave (const char *str);
extern int surface_delete(int n_user_old);
extern int surface_free(struct surface *surface_ptr);
extern struct surface *surface_search(int n_user, int *n, int print);
extern int temperature_free (struct temperature *temperature_ptr);
extern struct temperature *temperature_search(int n_user, int *n);
extern int trxn_copy (struct reaction *rxn_ptr);
...
#endif
#ifdef PHREEQCI_GUI
extern int copy_lfn_gui(char *token_ptr, char **ptr, int *l);
extern int error_checking_gui(void);
extern int get_logical_line_gui(FILE *fp, int *l);
#endif
#if defined(SWIG_SHARED_OBJ)
extern int error_checking_gui(void);
extern void swig_open_selected_output(const char* filename);
#endif

static LDBLE dummy;
static int check_line_return;
  ...
  */
	for (;;) {
		if (next_keyword >= 0) {
			...
			if (next_keyword != 49 && database_file == NULL) first_read_input = FALSE;
		}
		switch(next_keyword) {
 ...
 */
	...
	exchange[n].n_user = n_user;
	exchange[n].n_user_end = n_user_end;
	exchange[n].description = description;
	exchange[n].solution_equilibria = FALSE;
	exchange[n].related_phases = FALSE;
	exchange[n].related_rate = FALSE;
	if (use.exchange_in == FALSE) {
		use.exchange_in = TRUE;
		use.n_exchange_user = n_user;
	}
	exchange[n].comps = (struct exch_comp *) PHRQ_malloc(sizeof (struct exch_comp));
 ...
 *   Initialize
 */
	gas_phase[n].n_user = n_user;
	gas_phase[n].n_user_end = n_user_end;
	gas_phase[n].solution_equilibria = FALSE;
	gas_phase[n].total_p = 1.0;
	gas_phase[n].total_moles = 0.0;
	gas_phase[n].volume = 1.0;
	gas_phase[n].temperature = 298.15;
	gas_phase[n].new_def = TRUE;
	gas_phase[n].type = PRESSURE;
	gas_phase[n].description = description;
	gas_phase[n].comps = PHRQ_malloc((size_t) sizeof(struct gas_comp));
 ...
 *   Defaults
 */
	kinetics[n].n_user = n_user;
	kinetics[n].n_user_end = n_user_end;
	kinetics[n].description = description;
	kinetics[n].steps = PHRQ_malloc((size_t) sizeof(LDBLE));
	if (kinetics[n].steps == NULL)  malloc_error();
	kinetics[n].step_divide = 1.0;
	kinetics[n].count_steps = 0;
	kinetics[n].comps = PHRQ_malloc((size_t) sizeof(struct kinetics_comp));
	if (kinetics[n].comps == NULL) malloc_error();
	kinetics[n].count_comps = 0;
	kinetics[n].totals = NULL;
	kinetics[n].units = string_hsave("sec");
  	kinetics[n].rk = 3;
	kinetics[n].bad_step_max = 500;
	kinetics[n].use_cvode = FALSE;
 ...
 */
	...
	pp_assemblage[n].n_user_end = n_user_end;
	pp_assemblage[n].description = description;
/*
 *   Read phases
 */
	count_pure_phases = 0;
 ...
 */
	...
	for (;;) {
		...
		switch (opt) {
		    ...
		    case 0:                 /* file name */
#ifdef PHREEQCI_GUI
			copy_lfn_gui(file_name, &next_char, &l);
#else
			/* copy_token(file_name, &next_char, &l); */
			string_trim(next_char);
			strcpy(file_name, next_char);
#endif
#if defined(PHREEQCI_GUI) || defined(SWIG_SHARED_OBJ)
			if (error_checking_gui() == TRUE)
			{
#if defined(SWIG_SHARED_OBJ)
				swig_open_selected_output(file_name);
#else
			...
			if( (punch_file = fopen(file_name, "w")) == NULL) {
				...
				free_check_null(selected_output_file);
				selected_output_file = string_duplicate(file_name);
				/*setbuf(punch_file, NULL);*/
			}
#if defined(PHREEQCI_GUI) || defined (SWIG_SHARED_OBJ)
			}
#endif
...
#if defined(SWIG_SHARED_OBJ)
		if (punch_file == NULL) {
			swig_open_selected_output("selected.out");
		}
#else
		if (selected_output_file_name == NULL)
		{
			selected_output_file_name = string_duplicate("selected.out");
		}
#endif
	}
	else
	...
	if (punch_file == NULL) {
/*
 *  Open file first time, either default or read name
 */
		if( (punch_file = fopen("selected.out", "w")) == NULL) {
			sprintf(error_string, "Can't open file, %s.", "selected.out");
			input_error++;
			error_msg(error_string, CONTINUE);
		} else {
			free_check_null(selected_output_file);
			selected_output_file = string_duplicate("selected.out");
			/*setbuf(punch_file, NULL);*/
		}
#if defined(PHREEQCI_GUI) || defined(SWIG_SHARED_OBJ)
	}
 ...
 *   Default values
 */
	surface[n].n_user = n_user;
	surface[n].n_user_end = n_user_end;
	surface[n].new_def = TRUE;
        surface[n].diffuse_layer = FALSE;
        surface[n].edl = TRUE;
	surface[n].only_counter_ions = FALSE;
	surface[n].description = description;
	surface[n].solution_equilibria = FALSE;
	surface[n].related_phases = FALSE;
	surface[n].related_rate = FALSE;
/*
 *   Malloc one surface_comp structure
 */
	surface[n].comps = (struct surface_comp *) PHRQ_malloc( (size_t) sizeof (struct surface_comp));
	if (surface[n].comps == NULL) malloc_error();
	surface[n].comps[0].master = NULL;
	surface[n].comps[0].totals = NULL;
	surface[n].comps[0].phase_name = NULL;
	surface[n].comps[0].rate_name = NULL;
	surface[n].comps[0].charge = -1;
	surface[n].comps[0].moles = 0;
	surface[n].comps[0].cb = 0;
/*
 *   Malloc one charge structure
 */
	surface[n].charge = (struct surface_charge *) PHRQ_malloc( (size_t) sizeof (struct surface_charge));
	...
	do {
		...
		if (pr.echo_input == TRUE) {
			if ((print == TRUE && i != EOF) || i == KEYWORD) {
				...
				if (phast == TRUE && phreeqc_mpi_myself == 0) {
 ...
 */
	...
	char c;
#ifdef PHREEQCI_GUI
	if ((i = get_logical_line_gui(fp, l)) != 0) return i;
#endif
	i = 0;
	while ((j = getc(fp)) != EOF) {
		c = (char) j;
 ...
 */
	...
	if (j == EOF) {
		...
		if (find_option(&(option[1]), &opt, opt_list, count_opt_list, FALSE) == OK) {
			...
			*next_char = opt_ptr;
			if (pr.echo_input == TRUE) {
				if(database_file == NULL) output_msg(OUTPUT_MESSAGE, "\t%s\n", line_save);
			}
		} else {
			if(database_file == NULL) output_msg(OUTPUT_MESSAGE, "\t%s\n", line_save);
 ...
 *   Initialize
 */
	s_s_assemblage[n].n_user = n_user;
	s_s_assemblage[n].n_user_end = n_user_end;
#ifdef SKIP
	s_s_assemblage[n].solution_equilibria = FALSE;
	s_s_assemblage[n].type = PRESSURE;
#endif
	s_s_assemblage[n].description = description;
	s_s_assemblage[n].s_s = PHRQ_malloc((size_t) sizeof(struct s_s));
	if (s_s_assemblage[n].s_s == NULL) malloc_error();
	s_s_assemblage[n].count_s_s = 0;
	s_s_assemblage[n].new_def = TRUE;
	count_s_s = 0;
/*
 *   Set use data to first read
 */
