...
static char const rcsid[] = "$RCSfile: basicsubs.c,v $  $Revision: 2.14 $";
...
extern int add_other_logk(LDBLE *source_k, int count_add_logk, struct name_coef *add_logk);
...
extern int select_log_k_expression(LDBLE *source_k, LDBLE *target_k);
...
/* ---------------------------------------------------------------------- */
{
	...
	if (use.kinetics_ptr == NULL) return(0);
