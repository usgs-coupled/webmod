...
static char const rcsid[] = "$RCSfile: basic.c,v $  $Revision: 2.41 $";
...
extern int fpunchf(const char* name, const char* format, ...);
extern int fpunchf_user(int user_index, const char* format, ...);

extern LDBLE tc_x, rate_m, rate_m0, *rate_p, rate_time, rate_moles;
extern int count_rate_p;
extern LDBLE activity (char *species_name);
extern LDBLE calc_logk_s(char *name);
extern LDBLE calc_logk_n(char *name);
extern LDBLE calc_logk_p(char *name);
extern LDBLE diff_layer_total(char *total_name, char *surface_name);
extern LDBLE equi_phase (char *phase_name);
extern void *free_check_null(void *ptr);
extern LDBLE kinetics_moles (char *kinetics_name);
extern LDBLE find_gas_comp (char *gas_name);
extern LDBLE find_misc1 (char *s_s_name);
extern LDBLE find_misc2 (char *s_s_name);
extern LDBLE find_s_s_comp (char *s_s_name);
extern LDBLE get_calculate_value(char *name);
extern LDBLE log_activity (char *species_name);
extern LDBLE log_molality (char *species_name);
extern void malloc_error(void);
extern LDBLE molality (char *species_name);
extern int replace(const char *str1, const char *str2, char *str);
extern LDBLE saturation_ratio (char *phase_name);
extern int saturation_index (char *phase_name, LDBLE *iap, LDBLE *si);
extern struct save_values *save_values_bsearch (struct save_values *k, int *n);
extern int save_values_store(struct save_values *s_v);
extern char * string_duplicate (const char *token);
extern char *string_pad (char *str, int i);
extern int string_trim (char *str);
extern int string_trim_right (char *str);
extern int string_trim_left (char *str);
extern LDBLE sum_match_gases (char *template, char *name);
extern LDBLE sum_match_species (char *template, char *name);
extern LDBLE sum_match_s_s (char *template, char *name);
extern LDBLE surf_total(char *total_name, char *surface_name);
LDBLE system_total(char *total_name, LDBLE *count, char ***names, char ***types, LDBLE **moles);
extern LDBLE total (char *total_name);
...
extern void hdestroy_multi(HashTable *HashTable_ptr);
