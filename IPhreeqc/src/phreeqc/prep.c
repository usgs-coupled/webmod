...
static char const rcsid[] = "$RCSfile: prep.c,v $  $Revision: 2.13 $";
...
extern struct master_isotope  *master_isotope_search (char *name);
...
extern LDBLE rxn_find_coef(struct reaction *r_ptr, const char *str);
extern int rxn_free(struct reaction *rxn_ptr);
extern int s_s_prep(LDBLE t, struct s_s *s_s_ptr, int print);
extern int species_list_compare_master (const void *ptr1, const void *ptr2);
...
extern LDBLE trxn_find_coef(const char *str, int start);
extern int trxn_print (void);
extern int trxn_reverse_k(void);
extern int trxn_swap (const char *token);
extern struct unknown *unknown_alloc (void);
extern int unknown_free(struct unknown *unknown_ptr);
