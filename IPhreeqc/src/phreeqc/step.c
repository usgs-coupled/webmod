...
static char const rcsid[] = "$RCSfile: step.c,v $  $Revision: 2.11 $";
...
#ifdef SKIP
extern struct pe_data *pe_data_alloc(void);
#endif
extern int pe_data_free (struct pe_data *pe_data_ptr);
extern struct phase *phase_bsearch (char *name, int *i, int print);
extern int pp_assemblage_copy(struct pp_assemblage *pp_assemblage_old_ptr, struct pp_assemblage *pp_assemblage_new_ptr, int n_user_new);
...
extern int step_save_exch(int i);
extern int step_save_surf(int i);
